   ...
   :  G4VPhysicsConstructor(name)
{
  ...
  param->SetStepFunctionLightIons(0.1, 0.001*um);
  param->SetStepFunctionIons(0.1, 0.001*um);
  param->SetFluo(true);
...
void PhysListEmStandardNR::ConstructProcess()
{
  ...
  // defaults are to allow recoils, G4bool GenerateRecoils=true, G4double RecoilCutoff=100.0*eV, G4double PhysicsCutoff=10.0*eV
  G4ScreenedNuclearRecoil* nucr = new G4ScreenedNuclearRecoil();
  //set the minimum energy (per nucleon) at which recoils can be generated, and the energy (per nucleon) below which all ions are stopped. \param energy energy per nucleon 
  G4double recoil_cutoff = 0.0*eV;
  nucr->SetRecoilCutoff(recoil_cutoff);


  G4double energyLimit = 100.*MeV; // used to be 100 MeV - making 1000 MeV gave you data out further which is good
